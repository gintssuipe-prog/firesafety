...
class Finish extends Phaser.Scene {
  ...
  create() {
    ...
    this._topTitle.setVisible(false);
  ...
  drawEntryRow() {
    ...
    // hide save + input if not needed
    if (!this._hasEntryRow || !this._panelRect) {
      if (this._btnSave) {
        this._btnSave.bg.setVisible(false);
        this._btnSave.t.setVisible(false);
        this._btnSave.bg.disableInteractive();
    ...
    const y = this._entryY || (panel.y + 22);
    const h = 44;

    const pad = 14;
    const xRank = panel.x + pad;
    const xName = panel.x + Math.round(panel.w * 0.52);
    const xTime = panel.x + panel.w - pad;

    // subtle entry row background
    const g = this.add.graphics();
    g.fillStyle(0xffffff, 0.10);
    g.fillRoundedRect(panel.x, y - h / 2, panel.w, h, 10);
    this._entryGfx = g;

    // rank (left)
    const rText = this.add.text(xRank, y, String(this._insertRank || ""), {
      fontFamily: "Arial",
      fontSize: "18px",
      color: "#ffffff",
      fontStyle: "bold"
    ...
    // hint (right, small)
    const hint = this.add.text(xTime, y, "Ieraksti vārdu", {
      fontFamily: "Arial",
      fontSize: "13px",
      color: "#d7e1ee"
    }).setOrigin(1, 0.5);

    this._entryTexts.push(rText, hint);

    // show SAVE button (small) + enable
    if (this._btnSave) {
    ...
    // DOM input centered in the row, does NOT scroll
    this.addNameEntryUI(this._insertRank, this._pendingTimeSec, xName, y);
  }

  syncDomContainer() {
    // Phaser DOM container nav vienmēr sinhrons ar FIT + autoCenter.
    // Pielāgojam to canvas reālajiem ekrāna bounds, lai input ne-aizbrauc.
    const dc = this.scale && this.scale.domContainer;
    const b = this.scale && this.scale.canvasBounds;
    if (!dc || !b) return;
    dc.style.position = "absolute";
    dc.style.left = `${Math.round(b.left)}px`;
    dc.style.top = `${Math.round(b.top)}px`;
    dc.style.width = `${Math.round(b.width)}px`;
    dc.style.height = `${Math.round(b.height)}px`;
    dc.style.pointerEvents = "none"; // tikai konkrētie DOM elementi saņems events
  }

layout() {
    this.syncDomContainer();
    const W = this.scale.width;
    const H = this.scale.height;

    // title/sub/status
    this._title.setPosition(W / 2, 72);
    this._sub.setPosition(W / 2, 108);
    // _topTitle is hidden (legacy)
    this._statusText.setPosition(W / 2, 138);

    // bottom buttons
    const btnY = H - 64;
    const gap = 26;

    const totalW = 180 * 2 + gap;
    const leftX = Math.round(W / 2 - totalW / 2 + 90);
    const rightX = leftX + 180 + gap;

    this._btnRestart.setPosition(leftX, btnY);
    this._btnExit.setPosition(rightX, btnY);

    // panel sizing
    const panelW = Math.min(360, Math.max(280, W - 40));
    const panelX = Math.round((W - panelW) / 2);

    const panelTopY = 160; // zem statusa
    const bottomY = btnY - 48;
    const panelH = Math.max(160, bottomY - panelTopY);

    const entryH = this._hasEntryRow ? 44 : 0;
    const headerH = 30;

    this._panelRect = { x: panelX, y: panelTopY, w: panelW, h: panelH };
    this._entryY = panelTopY + (entryH ? entryH / 2 : 0);
    this._headerY = panelTopY + entryH;

    const bodyY = panelTopY + entryH + headerH;
    const bodyH = Math.max(90, panelH - entryH - headerH);

    this._tableRect = { x: panelX, y: bodyY, w: panelW, h: bodyH };

    // mask only for scroll body
    if (this._tableMaskGfx) {
      this._tableMaskGfx.clear();
      this._tableMaskGfx.fillStyle(0xffffff, 1);
      this._tableMaskGfx.fillRect(this._tableRect.x, this._tableRect.y, this._tableRect.w, this._tableRect.h);
      this._tableMaskGfx.setVisible(false);
    }

    // SAVE small button aligned with entry row (not at bottom)
    if (this._btnSave) {
      const pad = 14;
      const saveX = panelX + panelW - pad - 64; // 128/2
      const saveY = this._entryY || (panelTopY + 22);
      this._btnSave.setPosition(saveX, saveY);
    }

    // redraw entry/header/scroll
    this.drawEntryRow();
    this.applyScroll();
  }

makeSmallButton(cx, cy, label, baseColor = 0x1f3a52, pressColor = 0x2a587c) {
  const btnW = 128;
  const btnH = 40;
  ...
  applyScroll() {
    ...
    // DOM input ir fiksēts virs skrollējamā saraksta (neskrollējas)
    for (const node of this._domNodes) {
      if (!node) continue;
      if (node._baseX != null) node.x = node._baseX;
      if (node._baseY != null) node.y = node._baseY;
    }
}
  }
...
// header (fiksēts virs skrollējamā saraksta)
if (this._headerObjs) {
  for (const o of this._headerObjs) { try { o.destroy(); } catch(e) {} }
}
this._headerObjs = [];

const headerY = (this._headerY != null) ? this._headerY : (rect.y - 30);
const xRank = rect.x + 16;
const xName = rect.x + 92;
const xTime = rect.x + rect.w - 16;

const headerBg = this.add.rectangle(rect.x + rect.w / 2, headerY + headerH / 2, rect.w, headerH, 0x1f3a52, 0.25);
const hStyle = { fontFamily: "Arial", fontSize: "16px", color: "#ffffff", fontStyle: "bold" };

const t1 = this.add.text(xRank, headerY + 6, "Vieta", hStyle).setOrigin(0, 0);
const t2 = this.add.text(xName, headerY + 6, "Vārds", hStyle).setOrigin(0, 0);
const t3 = this.add.text(xTime, headerY + 6, "Laiks", hStyle).setOrigin(1, 0);

this._headerObjs.push(headerBg, t1, t2, t3);
...
// sagatavojam rindas renderēšanai
    ...
    // (UX) saglabāšanas rinda ir fiksēta virs skrollējamā saraksta, tabulā neievietojam tukšu rindu.
  ...
  submitScoreFromInput() {
    ...
    this.jsonp(url).then((res) => {
      ...
      this._statusText.setText("");
      this._canSave = false;
      // paslēpjam ievadi, lai vairs nepaliek aktīva
      this.cleanupDom();
      this._hasEntryRow = false;
      this.drawEntryRow();
      if (this._btnSave) {
        this._btnSave.bg.disableInteractive();
        this._btnSave.bg.setAlpha(0.6);
      }
