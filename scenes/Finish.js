...
class Finish extends Phaser.Scene {
  ...
  enableNameInput() {
    ...
    input.placeholder = 'Ieraksti vārdu';
    ...
    input.addEventListener('input', () => {
      // allow save only if trimmed not empty
  ...
  updateNameInputPosition() {
    ...
    const bounds = this.scale.canvasBounds; // DOMRect-like {left, top, width, height}
    ...
    const pad = 14;
    const inputX = er.x + pad + 56; // after rank column
    const inputY = er.y + er.h / 2;
    const inputW = Math.max(120, er.w - pad * 2 - 56 - 128 - 10); // rank + save + gap
    const inputH = 36;
