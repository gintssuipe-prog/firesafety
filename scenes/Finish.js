...
class Finish extends Phaser.Scene {
  ...
  create() {
    ...
    this._btnRestart = this.makeBigButton(0, 0, "ATSĀKT", 0x1f4a2c, 0x2a6a3b);
  ...
  addNameEntryUI(insertRank, timeSec, xName, rowCenterY) {
    ...
    if (!rect) return;
        ...
        // rādām statusā, nevis virs tabulas (mazāk trokšņa)
    this._statusText.setText(`TAVA VIETA TOP 50: #${insertRank} — IERAKSTI VĀRDU`);
    this._statusText.setColor("#ffffff");

    const inputX = xName + Math.min(260, Math.max(160, rect.w - 210)) / 2;
    const inputY = rowCenterY;
