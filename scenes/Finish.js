  ...
  renderTop(top, reason, timeSec) {
    ...
    if (showInsert && myRowY !== null) {
      ...
      const doSave = async () => {
        ...
        try {
          const out = await this.jsonp(this.API_URL + "?action=submit&name=" + encodeURIComponent(name) + "&time=" + encodeURIComponent(timeSec) + "&token=" + encodeURIComponent(this.TOKEN));
